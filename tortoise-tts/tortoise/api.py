...
def do_spectrogram_diffusion(diffusion_model, diffuser, latents, conditioning_latents, temperature=1, verbose=True, desc=None, sampler="P", input_sample_rate=22050, output_sample_rate=24000):
    ...
    with torch.no_grad():
        ...
        with tracer.start_as_current_span("precompute_embeddings"):
        ...
        with tracer.start_as_current_span("denormalize_tacotron_mel"):
...
def migrate_to_device( t, device ):
    ...
    with tracer.start_as_current_span("move_to_device"):
        t = t.to(device)

    with tracer.start_as_current_span("do_gc"):
        do_gc()
...
class TextToSpeech:
    ...
    def __init__(self, autoregressive_batch_size=None, models_dir=MODELS_DIR, enable_redaction=True, device=None,
        ...
        use_deepspeed=False, disable_clvp=False, skip_autoregressive_model_preload=False
    ):
        """
        Constructor
        ...
        self.autoregressive_model_path = None
        self.autoregressive_model_hash = None

        if not skip_autoregressive_model_preload:
    ...
    def load_autoregressive_model(self, autoregressive_model_path, is_xtts=False):
        if self.autoregressive_model_path and os.path.samefile(self.autoregressive_model_path, autoregressive_model_path):
            return
        ...
        if self.autoregressive_model_hash and self.autoregressive_model_hash == new_hash:
            return
    ...
    def tts(self, text, voice_samples=None, conditioning_latents=None, k=1, verbose=True, use_deterministic_seed=None,
        ...
        """
        with tracer.start_as_current_span("tts_loading"):
            ...
            with tracer.start_as_current_span("load_ar"):
            ...
            with tracer.start_as_current_span("load_diffusion"):
            ...
            with tracer.start_as_current_span("load_tokenizer"):
                if tokenizer_json is None:
                    tokenizer_json = self.tokenizer_json
                elif tokenizer_json != self.tokenizer_json:
                    self.load_tokenizer_json(tokenizer_json)

        with tracer.start_as_current_span("tts_padding"):
            with tracer.start_as_current_span("tokenizer encode"):
                text_tokens = torch.IntTensor(self.tokenizer.encode(text)).unsqueeze(0)

            with tracer.start_as_current_span("migrate_to_device"):
                text_tokens = migrate_to_device( text_tokens, self.device )

            with tracer.start_as_current_span("pad"):
                text_tokens = F.pad(text_tokens, (0, 1))  # This may not be necessary.
            assert text_tokens.shape[-1] < 400, 'Too much text provided. Break the text up into separate segments and re-try inference.'

        with tracer.start_as_current_span("tts_conditioning_loading"):
            auto_conds = None
            if voice_samples is not None:
                with tracer.start_as_current_span("get_conditioning_latents"):
                ...
                with tracer.start_as_current_span("get_random_conditioning_latents"):
                    auto_conditioning, diffusion_conditioning = self.get_random_conditioning_latents()

        with tracer.start_as_current_span("tts_misc"):
        ...
        with torch.no_grad():
            with tracer.start_as_current_span("tts_autoregressive_loading"):
                samples = []
                ...
                with tracer.start_as_current_span("migrate_ar"):
                    self.autoregressive = migrate_to_device( self.autoregressive, self.device )

                with tracer.start_as_current_span("migrate_conditioning"):
                    auto_conditioning = migrate_to_device( auto_conditioning, self.device )

                with tracer.start_as_current_span("migrate_text_tokens"):
                    text_tokens = migrate_to_device( text_tokens, self.device )

            with tracer.start_as_current_span("tts_autoregressive_inference"):
            ...
            with tracer.start_as_current_span("tts_autoregressive_postprocessing"):
                ...
                for batch in tqdm(samples, desc="Fixing autoregressive outputs"):
                    for i in range(batch.shape[0]):
                        batch[i] = fix_autoregressive_output(batch[i], stop_mel_token)

            if self.clvp:
                with tracer.start_as_current_span("tts_clvp_cvvp_loading"):
                    clip_results = []
                ...
                with tracer.start_as_current_span("tts_clvp_cvvp_inference"):
                ...
                with tracer.start_as_current_span("tts_clvp_cvvp_misc_2"):
                    samples = torch.cat(samples, dim=0)
                    best_results = samples

            if get_device_name() == "dml":
                with tracer.start_as_current_span("migration_misc"):
                ...
                with tracer.start_as_current_span("migration_misc_2"):
            ...
            with tracer.start_as_current_span("tts_diffusion_loading"):
                with tracer.start_as_current_span("auto_conditioning_repeat"):
                    a = auto_conditioning.repeat(k, 1)

                with tracer.start_as_current_span("text_tokens_repeat"):
                    b = text_tokens.repeat(k, 1)

                with tracer.start_as_current_span("t1"):
                    t1 = torch.tensor([text_tokens.shape[-1]], device=text_tokens.device)

                with tracer.start_as_current_span("t2"):
                    t2 = torch.tensor([best_results.shape[-1]*self.autoregressive.mel_length_compression], device=text_tokens.device)

                with tracer.start_as_current_span("best_latents"):
                    best_latents = self.autoregressive(a, b,
                        t1, best_results,
                        t2,
                        return_latent=True, clip_inputs=False)

                with tracer.start_as_current_span("migrate_diffusion_conditioning"):
                    diffusion_conditioning = migrate_to_device( diffusion_conditioning, self.device )

                if get_device_name() == "dml":
                    with tracer.start_as_current_span("migrate_dml"):
                        self.autoregressive = migrate_to_device( self.autoregressive, self.device )
                    ...
                    with tracer.start_as_current_span("migrate_autoregressive"):
                        if not self.preloaded_tensors:
                            self.autoregressive = migrate_to_device( self.autoregressive, 'cpu' )

                with tracer.start_as_current_span("migrate_diffusion"):
                    self.diffusion = migrate_to_device( self.diffusion, self.device )

                with tracer.start_as_current_span("migrate_vocoder"):
                    self.vocoder = migrate_to_device( self.vocoder, self.device )
                ...
                wav_candidates = []
            with tracer.start_as_current_span("tts_diffusion_inference"):
                for b in range(best_results.shape[0]):
                    ...
                    with tracer.start_as_current_span("tts_vocoder_inference"):
                        wav = self.vocoder.inference(mel)

                    wav_candidates.append(wav)

                if not self.preloaded_tensors:
                    with tracer.start_as_current_span("migrate_to_cpu"):
                        self.diffusion = migrate_to_device( self.diffusion, 'cpu' )
                        self.vocoder = migrate_to_device( self.vocoder, 'cpu' )

                with tracer.start_as_current_span("tts_diffusion_misc"):
                    with tracer.start_as_current_span("redact"):
