...
class GaussianDiffusion:
    ...
    @tracer.start_as_current_span(name="q_mean_variance")
    ...
    @tracer.start_as_current_span(name="q_sample")
    ...
    @tracer.start_as_current_span(name="q_posterior_mean_variance")
    ...
    @tracer.start_as_current_span(name="p_mean_variance")
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        with tracer.start_as_current_span(name="infer_conditioned_diffuser"):
            model_output = model(x, self._scale_timesteps(t), **model_kwargs)
        if self.conditioning_free:
            with tracer.start_as_current_span(name="infer_unconditioned_diffuser"):
    ...
    @tracer.start_as_current_span(name="p_sample")
    def p_sample(
        self,
        model,
        x,
    ...
    @tracer.start_as_current_span(name="sample_loop")
    ...
    @tracer.start_as_current_span(name="p_sample_loop")
    def p_sample_loop(
        self,
        model,
        shape,
    ...
    @tracer.start_as_current_span(name="p_sample_loop_progressive")
